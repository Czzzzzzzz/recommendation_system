...
class BaseModel:
    ...
    def _compute_data_loss(self):
        if self.hparams.loss == "cross_entropy_loss":
            ...
            #logits = tf.compat.v1.Print(logits, [logits], "debug_logits")
    ...
    def _active_layer(self, logit, activation, layer_idx=-1):
        ...
        #logit = tf.compat.v1.Print(logit, [logit[0]], "debug_dropout_layer_%d" % layer_idx, summarize=1000)
    ...
    def _dropout(self, logit, keep_prob):
        ...
        """
        # Note(caozheng03): if seed is not set, the result may be different for each run.
        return tf.nn.dropout(x=logit, rate=1 - (keep_prob))
        # return tf.nn.dropout(x=logit, rate=1 - (keep_prob), seed=self.seed)
        # return tf.nn.experimental.stateless_dropout(x=logit, rate=1 - (keep_prob), seed=[self.seed, 1])
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            with tf.compat.v1.variable_scope(
                ...
                for idx, layer_size in enumerate(layer_sizes):
                    ...
                    #curr_w_nn_layer = tf.compat.v1.Print(curr_w_nn_layer, [curr_w_nn_layer[0]], "debug_curr_w_nn_index=%d" % idx  )
                    ...
                    #curr_hidden_nn_layer = tf.compat.v1.Print(curr_hidden_nn_layer, [curr_hidden_nn_layer[0]], "debug_hidden_nn_layer=%d" % idx, summarize=1000)
